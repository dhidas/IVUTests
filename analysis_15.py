    ...
    with open(filename) as fi:
        ...
        for line in fi:
            ...
            for i in range(5, 9):
                V[i] = +V[i]
            for i in range(9, 13):
                V[i] = V[i]/(1000000./1048576.)/(1000./8.)
...
def runme(filename):
    ...
    run = filename[-4-8-1-6:-4]
    os.makedirs(os.path.dirname(filename) + '/plots/ana/'+run)
    odir = os.path.dirname(filename) + '/plots/ana/' + run
    make_plots_one_file (filename, time=[-1, 1e99], odir = odir)

    # Create html file Ana_run.html
    with open(os.path.dirname(filename)+'/Ana_'+run+'.html', 'w') as fo:
        fo.write('<html><body>\n')
        for i in range(1, 5):
            fo.write('<a href=plots/ana/'+run+'/data_'+run+'_'+str(i)+'.png>\n')
            fo.write('<img src=plots/ana/'+run+'/data_'+run+'_'+str(i)+'.png>\n')
